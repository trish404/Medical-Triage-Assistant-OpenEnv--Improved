...
"consent_assessment": textwrap.dedent("""
    You are an ED physician. Complete all 3 consent scenarios: CS001, CS002, CS003.
    Be EFFICIENT — use minimum steps per scenario.

    CS001 (competent adult, appendectomy):
      assess:CS001:understands_info -> assess:CS001:appreciates_situation ->
      assess:CS001:reasons_through_options -> assess:CS001:communicates_choice ->
      declare_capacity:CS001:has_capacity -> step:CS001:explain_procedure ->
      step:CS001:document_consent -> consent_path:CS001:informed_consent -> finalize:CS001

    CS002 (unconscious, intubation, no surrogate):
      assess:CS002:understands_info -> assess:CS002:appreciates_situation ->
      assess:CS002:reasons_through_options -> assess:CS002:communicates_choice ->
      declare_capacity:CS002:lacks_capacity -> step:CS002:invoke_emergent_exception ->
      consent_path:CS002:emergent_exception -> finalize:CS002

    CS003 (15yo minor, transfusion, parents coming):
      assess:CS003:understands_info -> assess:CS003:appreciates_situation ->
      assess:CS003:reasons_through_options -> assess:CS003:communicates_choice ->
      declare_capacity:CS003:lacks_capacity -> step:CS003:contact_guardian ->
      step:CS003:obtain_assent -> consent_path:CS003:assent_minor -> finalize:CS003

    Follow these sequences exactly. Reply with EXACTLY ONE action per turn.
""").strip(),
